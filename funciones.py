...
from scipy.signal import butter, zpk2sos, sosfilt, lfilter, bilinear, tf2sos
...
def frec_lim(frec_cen, n_oct=1.0):
    """ Devuelve las frecuencias límites (inferior y superior), para
...
def fto_lim(fto_nom=fto_nom):
    ...
    tercio de octava. Permite el ingreso del valor nominal de las frecuencias
    centrales para las bandas deseadas, 'fto_nom'. Por defecto 25 Hz < f < 10 kHz. """
...
def foct_lim(foct_nom=foct_nom):
    """ Entrega las frecuencias límite para el diseño de los filtros de
    octava. Permite el ingreso del valor nominal de las frecuencias centrales para las
...
z_a = np.append(z_c, [0,0])
p_a = np.array([-2*np.pi*20.6, -2*np.pi*20.6,
     -2*np.pi*107.7, -2*np.pi*737.9,
     -2*np.pi*12194.0, -2*np.pi*12194.0])
k_a = (10**(2/20))*p_a[4]**2
...
sos_C = zpk2sos(*zpk_bil(z_c, p_c, k_c), pairing='keep_odd')
sos_A = zpk2sos(*zpk_bil(z_a, p_a, k_a), pairing='keep_odd') 
...
def filt_A(x):
    ...
    sos_A2 = ([[ 0.23430989,  0.46861978,  0.23430989,  1.0 , -0.22455845, 0.01260662],
       [1.0, -2.0, 1.0, 1.0, -1.89387049, 0.89515977],
       [1.0, -2.0, 1.0, 1.0, -1.99461446, 0.99462171]])
    return sosfilt(sos_A2, x)

def filt_C(x):
    """ Devuelve la señal posterior al proceso de filtrado según
    ponderación 'C'. Recibe la señal de entrada (en dimensión temporal). """
    sos_C2 = ([[ 0.1978894 ,  0.3957788 ,  0.1978894 , 1.0, -0.22455845, 0.01260662],
       [1.0 , -2.0,  1.0 , 1.0, -1.99461446, 0.99462171]])
    return sosfilt(sos_C2, x)
...
def gen_impulsive(fs=fs):
    """Generación del filtro de ponderación temporal 'Fast'. """
    b, a = bilinear(np.poly1d([1]), np.poly1d([0.035, 1]), fs)
    return b, a

def slow(x, fs=fs):
    #Aplicación de la ponderación temporal 'Slow'.
    #b, a = gen_slow(fs)
    b = ([1.04165582e-05, 1.04165582e-05])
    a = ([1.0, -0.99997917])
...
def fast(x, fs=fs):
    ...
    #b, a = gen_fast(fs)
    b = np.array([8.33263895e-05, 8.33263895e-05])
    a = np.array([1.0, -0.99983335])
    return np.sqrt(lfilter(b, a, x**2))

def impulsive(x, fs=fs):
    """ Aplicación de la ponderación temporal 'Impulsive'. """
    #b, a = gen_impulsive(fs)
    b = np.array([0.00029753, 0.00029753])
    a = np.array([ 1.0, -0.99940494])
    return np.sqrt(lfilter(b, a, x**2))
"""
...
def rms_pond_t(x,pond_t,t=1.0, fs=fs):
    """ Cálculo de niveles RMS para la señal de entrada 'x', fragmentando
    de acuerdo al intervalo de tiempo 't' indicado. """
    if pond_t == 1.0:
        x = slow(x)
...
def niveles(x, cal, ncal):
    ...
    #y_2 = 20*np.log10(np.abs(x/(0.00002*cal)))
    #Leq = 10*(np.log10((1/(len(y_2)))*np.sum(10**(y_2/10))))
    return 10*np.log10(x**2/cal**2)+ncal
...
def Lpeak(x,vcal,ncal):
    """Obtención del valor pico luego de aplicar
    ponderación frecuencial."""
    return np.amax((niveles(x,vcal,ncal))) # Vieja resolución
    #return np.amax((niveles(levels,vcal,ncal)))

def Lmax(x,vcal,ncal):
    """Obtención del valor máximo RMS de la señal"""
    #x = slow(x)
    #levels = 10.0 * np.log10(integrate(x**2.0, 48000, 1) / (vcal*0.00002)**2.0)
    #return np.amax(levels)
    #return np.amax(niveles((slow(x)),vcal,ncal))
    #signal = niveles(rms_t(x),vcal,ncal)
    signal = niveles((slow(x)),vcal,ncal)
    signal = signal[48000:]
    return np.amax(signal), np.amin(signal)

def Lmin(x,vcal,ncal):
    """Obtención del valor mínimo RMS de la señal"""
    x = slow(x)
    #levels = 10.0 * np.log10(integrate(x**2.0, 48000, 1) / (vcal*0.00002)**2.0)
    #return np.amin(levels)
    #return np.amin(niveles((slow(x)),vcal,ncal))
    #signal = niveles(rms_t(x),vcal,ncal)
    signal = niveles((slow(x)),vcal,ncal)
    return np.amin(signal[48000:])
